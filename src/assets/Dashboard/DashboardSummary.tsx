  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        setSummary({
          pvEnergy,
          loadEnergy: Number(data.loadEnergy ?? 0),
          batCharge: Number(data.batCharge ?? 0),
          batDischarge: Number(data.batDischarge ?? 0),
          gridImport: Number(data.gridImport ?? 0),
          gridExport: Number(data.gridExport ?? 0),
        });

        setOutputFreq(
          data.outputFreq !== undefined && data.outputFreq !== null
            ? Number(data.outputFreq)
            : null
        );
        ...
        setIrradiance(Number(data.irradiance ?? 0));
        setBackplaneTemp(Number(data.backplaneTemp ?? 0));
      } catch (error) {
